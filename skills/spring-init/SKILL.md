...
## §0 실행 컨텍스트 주의 (Claude Code 전용)

- **Bash 간 CWD 는 영속되지 않는다.** `cd <dir>` 로 이동 후 다음 Bash 호출은
  원래 CWD 로 리셋된다. `cd <dir> && ...` 를 한 호출 안에서 완결하거나,
  이후 Write/Bash 모두 **절대 경로**로 지정한다.
- 디렉터리 생성 후 파일 작성 패턴:
  ```bash
  mkdir -p /absolute/path/to/project
  ```
  이후 모든 Write 는 `/absolute/path/to/project/...` 절대 경로로.
- `cd project && Write(file)` 패턴은 `project/project/file` 중첩 생성을
  유발하므로 **사용 금지**.
...
> `bootVersion` 은 스크립트가 `.RELEASE` · `.BUILD-SNAPSHOT` 등의 접미사를
> 자동으로 제거한 **정제값**이다 (예: `4.0.5.RELEASE` → `4.0.5`). 그대로 사용하면 된다.
> 원본은 `bootVersionRaw` 에 보존된다.
...
1. `fetch-latest-versions.sh` 로 `bootVersion` 을 확보 (반드시 이 값만 사용)
   - **금지**: 사용자가 구두/프롬프트로 알려준 버전 숫자 사용.
     사용자 제공 버전도 학습 데이터 추정과 동일하게 신뢰할 수 없으므로 **금지**.
...
- **금지**: 사용자에게 "버전 숫자를 직접 알려주시면 진행 가능" 같은 우회를 제안하는 것.
  사용자가 제공하는 버전도 학습 데이터 추정과 신뢰 수준이 동일하다.
- 대안: 네트워크 복구 후 재실행, 또는 오프라인 캐시(있을 경우)로 전환 후 사용자 승인 받기
- 복구 커맨드를 안내할 때 `/etc/hosts` 등 설정 파일은 **패턴 기반 편집**을 사용한다
  (예: `sudo sed -i '' '/start.spring.io/d' /etc/hosts`). 라인 번호는 환경 의존적이므로 금지.
...
#### 3-a. Gradle wrapper 확인 및 갱신

zip 으로 생성된 프로젝트에는 wrapper 가 포함되어 있다.
`gradle-conventions.md` §0 Boot-Gradle 호환 표를 보고 로컬 Gradle 버전이 미달이면:
...
스크립트는 **절대 경로**로 호출한다 (§0 CWD 비영속 규칙 참조):

```bash
bash /absolute/path/to/.claude/skills/spring-init/scripts/apply-package-structure.sh \
  /absolute/path/to/project <style> <basePackagePath>
```
