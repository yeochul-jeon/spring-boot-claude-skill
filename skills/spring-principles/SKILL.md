...
- [ ] VO는 setter가 없다 (불변) — `record` 또는 생성자만 허용
...
```bash
# A: @Autowired 필드 주입 탐지
grep -rn "@Autowired" <SRC>

# B: Controller 메서드가 Entity를 직접 반환하는지 (타입명 기반 — 수동 확인 병행)
grep -rn "public.*Entity\b\|public Order\|public Product\|public Member" <SRC>/*/controller/

# C: @Transactional이 Controller에 잘못 위치
grep -rn "@Transactional" <SRC>/*/controller/

# F: VO에 setter 존재 (불변성 위반)
grep -rn "public void set" <SRC>/*/domain/Money.java <SRC>/*/domain/*Vo.java 2>/dev/null

# G: private final 필드 전무 (생성자 주입 미사용)
grep -rn "private final" <SRC> | wc -l   # 0 이면 경고
```

결과가 있으면 해당 `templates/` Before→After 를 적용한다.
