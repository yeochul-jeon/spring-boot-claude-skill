...
## 케이스 8: spring-testing 패턴 준수

- **프롬프트**: "회원 CRUD REST API Controller·Service·Repository 테스트 + 통합 테스트 작성해줘"
- **검증**:
  - [ ] 통합 테스트에 H2 없음 — TestContainers 로 실제 DB 사용
  - [ ] `@DataJpaTest` 사용 시 `@AutoConfigureTestDatabase(replace = NONE)` 적용
  - [ ] `testcontainers:junit-jupiter` 의존성 포함
  - [ ] `withReuse(true)` 컨테이너 재사용 설정
  - [ ] 도메인별 Fixtures / ObjectMother 패턴 사용 (인라인 `new 도메인(...)` 반복 없음)
  - [ ] `@SpringBootTest` 남용 없음 (슬라이스로 커버 가능한 범위는 슬라이스 사용)
...
| 2026-04-22 | 5 | BASELINE | empirical 검증 — 결함 5건 수집, 탐지 공백 3건(S_DP·S4·S5) + spring-web 교차 2건(W2·W_login); grep 자동화 섹션 추가 완료 |
| 2026-04-22 | 8 | BASELINE | empirical 검증 — 결함 4건 수집, 탐지 공백 3건(T1·T3·T4/T8) + 구조 공백 1건(T_Fixture); grep 자동화 섹션 추가 완료 |
| 2026-04-22 | 8 | PASS | 2차 회전 — SKILL 로드 상태; T1~T8 전부 기대값 충족, 결함 0건; codex 리뷰 clean |
| 2026-04-22 | 1~8 전체 | PASS | 통합 리그레션 (retro #20) — §A meta-regression 0건; §B test-retro18-v2 T1~T7 PASS; §C @MockitoBean 마이그레이션 유지 |
| 2026-04-22 | 6 (W5a 보강) | PASS | retro #21 — W5a grep 을 multi-line 인지 스캐너(`tr + grep -oE`) 로 교체; 스킬 SKILL.md 에 single-line 권장 컨벤션 추가; 실측 실패 0건(latent) |
| 2026-04-23 | §A 확장 (6개 스킬 × 3~5규칙) | PASS | retro #22 — 21개 규칙 자기참조 위반 0건; 라이브러리 버전(testcontainers/mapstruct) 허용 예외 적용 |
| 2026-04-23 | §B 2차 회전 (principles/web/persistence/security) | PASS | retro #22 — 4개 스킬 정적 재검증 전부 PASS; 자기참조 위반 없음 |
| 2026-04-23 | 신규 스킬 뼈대 4건 (batch/cache/observability/async) | INFO | retro #22 — v0.1 초안 작성 완료; sandbox/empirical 없음 (Phase B/C 이관) |
| 2026-04-23 | spring-batch BASELINE (memberExportJob) | FAIL (2건) | retro #23 — B1 false-neg (type witness), B3 false-pos (DataSource 분리 미확인); grep 패턴 수정 + references anti-pattern 추가 |
| 2026-04-23 | spring-batch 2차 회전 | PASS | retro #23 — B1~B4 + spring-principles 전부 PASS; 결함 0건 |
| 2026-04-23 | spring-cache BASELINE (productCache) | PASS (탐지 공백 2건) | retro #24 — C1~C4 실제 결함 0건; C3_GREP_YML(yml TTL 미탐지)·C3_CAFFEINE_TTI(expireAfterAccess 미탐지) 탐지 공백 수정 |
| 2026-04-23 | spring-cache 2차 회전 | PASS | retro #24 — C1~C4 + spring-principles 전부 PASS; unless 이중 방어 포함 |
| 2026-04-23 | spring-observability BASELINE (orderApi) | PASS (탐지 공백 3건) | retro #25 — O1~O5 실제 결함 0건; O5_YAML_SPLIT(sampling YAML 분리)·O1_SINGLE_QUOTE(단일따옴표 wildcard)·O4_BOOT34(logging.structured.format) 탐지 공백 → grep 패턴 보강 |
| 2026-04-23 | spring-observability 2차 회전 | PASS | retro #25 — O1~O5 + spring-principles 전부 PASS; @Order(HIGHEST_PRECEDENCE) + probability 명시 |
| 2026-04-23 | spring-async BASELINE (welcomeEmail+scheduler) | PASS (탐지 공백 3건+버그1건) | retro #26 — AS1~AS4 실제 결함 0건; AS5_EXCEPTION·AS1_DIRECT_THREAD·AS3_MULTILINE + AS1_HEAD_BUG 수정 |
| 2026-04-23 | spring-async 2차 회전 | PASS | retro #26 — AS1~AS5 + spring-principles 전부 PASS; @Async("taskExecutor") + exceptionally 포함 |
| 2026-04-23 | §A 확장 Phase D (batch/cache/observability/async A7~A10, 17건) | PASS | retro #27 — 21개 → 38개; 허용 맥락 2건(A10b echo·A10c bash 주석) grep -v 보강 |
| 2026-04-23 | §A Phase D2 codex 리뷰 반영 (medium 8건) | PASS | retro #28 — A7a/A8a/A9a/A10a Boot-only 인버트; A9b properties 비인용 탐지; A9d probability 강제; A10b/A10c 앵커 교체; 38개 재실행 PASS |
| 2026-04-23 | §A Phase D3 codex low 5건 + known-limitation 3건 | PASS | retro #29 — A7d `chunk\s*(`; A8b/A10d FQCN; A8d `@TimeToLive`/`time-to-live`; A10e `handleAsync`; A7b/A7c/A8c/A9c 구조적 한계 주석 |
